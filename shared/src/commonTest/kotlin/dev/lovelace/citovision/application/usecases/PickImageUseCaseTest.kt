...
class PickImageUseCaseTest {
    ...
    private fun image(mimeType: String, sizeBytes: Long) =
        SelectedImage(bytes = ByteArray(1), fileName = "muestra.jpg", mimeType = mimeType, sizeBytes = sizeBytes)

    @Test
    fun `given a valid JPG when picking then returns the image`() = runTest {
        // Given
    ...
    @Test
    fun `given the user cancels when picking then returns success with null`() = runTest {
        // Given
    ...
    @Test
    fun `given an unsupported format when picking then fails with UnsupportedFormat`() = runTest {
        // Given
    ...
    @Test
    fun `given an image over the size limit when picking then fails with TooLarge`() = runTest {
        // Given
    ...
    @Test
    fun `given the picker fails when picking then propagates the failure`() = runTest {
        // Given
